...
class gl2D():
    ...
    def glScroll(self, scrollX,scrollY):
        # Scroll the image
        if scrollX is not None:  self.glScrollX = scrollX
        if scrollY is not None:  self.glScrollY = scrollY
        self.glViewReady = False
    ...
    def setupGLviewing(self):
        ...
        # Scroll the picture in the x and y direction
        glTranslatef(self.glScrollX * (self.glXmax - self.glXmin) * self.glZoomval,
                     self.glScrollY * (self.glYmax - self.glYmin) * self.glZoomval, 0)
...
def gl2DBox(left, right, bottom, top, fill=False):
    if fill:
        glBegin(GL_POLYGON)
    else:
        glBegin(GL_LINE_STRIP)
...
def gl2DArrow(xcenter, ycenter, radius, fill=True, thzero = 0):
    faces = 3
    if fill:
        glBegin(GL_POLYGON);
    else:
    ...
    glVertex2f(xcenter + np.cos(thzero) * radius, ycenter + np.sin(thzero) * radius)
    for i in range(1, faces + 1):
        theta = i / faces * 2 * np.pi
        glVertex2f(xcenter + np.cos(thzero + theta) * radius, ycenter + np.sin(thzero + theta) * radius)
    glEnd();
